...
const getAllkaryawan = async (req, res) => {
  const getkaryawanQuery = "SELECT * FROM tb_karyawan";

  try {
    const { rows } = await pool.query(getkaryawanQuery);
    const Response = rows;
...
const getUndikaryawan = async (req, res) => {
  const getkaryawanQuery =
    "select a.* from tb_karyawan a where not exists (select 1 from tb_undian b where b.nik = a.nik)";
